...
ampGainPrms  = zeros(length(kernels),length(tfsLevitt),length(grDirs),length(sfs));
phasePrms    = zeros(length(kernels),length(tfsLevitt),length(grDirs),length(sfs));
ampGainOffs  = zeros(length(tfsLevitt),length(sfs));
phaseOffs    = zeros(length(tfsLevitt),length(sfs));
...
ampGainPrms(kernelInd,tfInd,dirInd,sfInd) = .9 * (B/BAt4Hz) * LGNSpFreqDep(sf,1);
phasePrms(kernelInd,tfInd,dirInd,sfInd) = phi;
ampGainOffs(tfInd,sfInd) = .9 * (BOFF/BAt4Hz) * LGNSpFreqDep(sf,1);
phaseOffs(tfInd,sfInd) = phiOFF;
...
for kernelInd=1:length(kernels)
    ...
    for sfInd=1:length(sfs)
        ...
        dirIndL=1;
        dirIndR=2;
        ampsONL=ampGainPrms(kernelInd,:,dirIndL,sfInd);
        phasesONL=phasePrms(kernelInd,:,dirIndL,sfInd);
        ampsONR=ampGainPrms(kernelInd,:,dirIndR,sfInd);
        phasesONR=phasePrms(kernelInd,:,dirIndR,sfInd);
        cpxYOffs = [ampGainOffs(:,sfInd).*cos(phaseOffs(:,sfInd)), ampGainOffs(:,sfInd).*sin(phaseOffs(:,sfInd))];
        ...
        grid minor;
        if row==1
            title(['sf=' num2str(sfs(sfInd)) ' c/d']);
        end
%        if col==1
